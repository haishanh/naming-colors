...
const d1 = JSON.stringify(
  fs.readFileSync(path.resolve(__dirname, 'assets/nike.en.json'), 'utf8')
);
const d2 = JSON.stringify(
  fs.readFileSync(path.resolve(__dirname, 'assets/nike.zh.json'), 'utf8')
);
const d3 = JSON.stringify(
  fs.readFileSync(path.resolve(__dirname, 'assets/css.en.json'), 'utf8')
);

const jp = (x) => `JSON.parse(${x})`;
const toInject = `window.__data__=[${jp(d1)},${jp(d2)},${jp(d3)}]`;
